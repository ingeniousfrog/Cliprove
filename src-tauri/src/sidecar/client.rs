...
impl SidecarClient {
    pub fn new(port: u16) -> AppResult<Self> {
        Self::with_timeout(port, Duration::from_secs(30))
    }

    pub fn with_timeout(port: u16, timeout: Duration) -> AppResult<Self> {
        let connect_timeout = Duration::from_secs(3).min(timeout);
